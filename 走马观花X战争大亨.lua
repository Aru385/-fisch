...
Button1 = "明白";
Duration = 15})
local window = library:new("走马观花X-战争大亨")
...
gn:Label("请先拿起要修改的枪械打一枪，然后取消装备，再打开超量子弹和超射速，否则无伤")
gn:Button("超量子弹",function()
    for i,v in next,lp.Backpack:GetDescendants() do
    if v.Name=='Settings' then
        require(v).Ammo=10000
    end
end
end)
gn:Button("超射速",function()
    for i,v in next,lp.Backpack:GetDescendants() do
    if v.Name=='Settings' then
        require(v).FireRate=10000
        require(v).BurstFireRate=10000
    end
end
end)
...
gn:Button("武器全自动",function()
    ...
    if v.Name=='Settings' then
        require(v).Mode="Auto"
        require(v).FireModes.Auto=true
        --require(v).FireModes.Semi=true
        --require(v).FireModes.ChangeFiremode=true
    end
end
end)
...
pl:Button("透视",function()
    for i, v in next, game.Players:GetPlayers() do
        if v ~= lp then
            ...
            esplabe.Text = "玩家用户名 : " .. tostring(v.Name)
        end
    end
end)